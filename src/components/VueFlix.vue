...
<template>
  <div class="vueflix">
    <div class="acoes-usuario">
      <div class="filtros">
        ...
        <div class="opcoes-filtros">
          <button class="botao" :class="{ ativo: selectedFilter === 'all' }" @click="selectedFilter = 'all'">
            Todos
          </button>
          <button class="botao" :class="{ ativo: selectedFilter === 'liked' }" @click="selectedFilter = 'liked'">
            Gostei
          </button>
          <button class="botao" :class="{ ativo: selectedFilter === 'disliked' }" @click="selectedFilter = 'disliked'">
            Não Gostei
          </button>
        </div>
      </div>
      ...
      <div class="novo-filme">
        <div v-if="isAddingNew" class="adicionar-filme">
          ...
          <input type="text" autocomplete="off" placeholder="URL da Imagem" v-model="newImageURL" />
          <input type="text" autocomplete="off" placeholder="Data de Lançamento" v-model="newReleaseDate" />
        ...
        <button v-else class="botao ativo" @click="isAddingNew = true">Adicionar Filme</button>
      </div>
    </div>
    ...
    <div class="filmes">
      <div v-for="(filme, index) in filteredMovies" class="filme">
        <div class="capa-container">
          <div class="acoes-filme">
            <button class="botao" :class="{ ativo: filme.like === true }" @click="setLike(index, true)">
              Gostei
            </button>
            <button class="botao danger" :class="{ ativo: filme.like === false }" @click="setLike(index, false)">
              Não Gostei
            </button>
      ...
      <p v-if="filteredMovies.length === 0" :style="{ flex: 1, textAlign: 'center', marginTop: '16px' }">
